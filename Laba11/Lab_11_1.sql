...
Set transaction Isolation level  Read Uncommitted
Begin Tran
Select count(*) From AUDITORIUM --6
------
Select count(*) From AUDITORIUM -- неподтв чтение (5)
------
Select count(*) From AUDITORIUM --6
Commit tran


--Такой же пример, только с другим уровнем изоляции (Read Committed), тут не допускает непотвержд чтение
Set transaction Isolation level  Read Committed
Begin Tran
Select count(*) From AUDITORIUM --6
------
Select count(*) From AUDITORIUM -- неподтв чтения нет, поэтому будет ожидание
------
Select count(*) From AUDITORIUM --6
Commit tran
...
--Такой же пример, только с другим уровнем изоляции (Read Committed), тут допускает неповтор чтение
Set transaction Isolation level  Read Committed
Begin Tran
Select count(*) From AUDITORIUM --6
-----
Select count(*) From AUDITORIUM --5
commit tran



--Такой же пример, только с другим уровнем изоляции (Repeatable read)

Insert into AUDITORIUM values('140-1', 'ЛК', 60, '140-1')
Set transaction Isolation level Repeatable Read
Begin Tran
Select Count(*) From AUDITORIUM --6
-----
